...
pub struct MdnsResponder {
    wifi_receiver: Receiver<Ipv4Addr>,
}
...
impl MdnsResponder {
    ...
    pub fn start(&mut self, hostname: &str) -> anyhow::Result<()> {
        let hostname_local = format!("{}.local", hostname);
        let domain_name = Name::new_unchecked(&hostname_local);

        info!("MdnsResponder waiting for ip address...");
        // This is a blocing call, we are relying on WifiManager to have sent an address before we call this (which it does).
        let ip_address: Ipv4Addr = self.wifi_receiver.recv()?;
        info!("MdnsResponder got address {}", &ip_address);
